...
if(!empty($_POST['check'])){
    ...
    if($_POST['pass2']== ''){
        $error['pass2'] ='blank'; 
    }elseif(!preg_match($alpha,$_POST['pass2'])){
        $error['pass2']= 'format';
    }
    ...
    if (!isset($error)) {
        $_SESSION['join'] = $_POST;  
        header('Location: member_regist_confirmation.php'); 
        exit();
    }
...
<html>
  ...
  <header>
    ...
    <div class="form_item"> 
        ...
        <div class="form_item_input"><span>登録後に自動採番</span></div>
    </div>
    <div class="form_item">
        <p class="form_item_label">氏名</p>
        <div class="form_item_input">
            <ul>
                <li>姓<input class="name" type="text" name="name_sei" value="<?php if( !empty($_POST['name_sei']) ){ echo $_POST['name_sei']; } ?>" ></li>
                <li>名<input class="name" type="text" name="name_mei" value="<?php if( !empty($_POST['name_mei']) ){ echo $_POST['name_mei']; } ?>"></li>
            </ul>   
        </div>  
    </div>
    ...
    <div class="address form_item">
      ...
      <p>それ以降の住所<input type="text" name="address" class="form_item_input" value="<?php if( !empty($_POST['address']) ){ echo $_POST['address']; } ?>"></p>
        </div>
    </div>
    ...
    <div>
        <div class="form_item">
            <p class="form_item_label">パスワード</p><input type="password" name="password" class="form_item_input form_item_input2" value="<?php if( !empty($_POST['password']) ){ echo $_POST['password']; } ?>" >
        </div> 
        ...
        <?php endif ?> 
        <div class="form_item">
            <p class="form_item_label">パスワード確認</p><input type="password" name="pass2" class="form_item_input form_item_input2" value="<?php if( !empty($_POST['pass2']) ){ echo $_POST['pass2']; } ?>" >
        </div>
        <?php if(!empty($error["pass2"]) && $error['pass2'] == 'blank'): ?>
                <p class="error">※パスワード確認は必須入力です。</p>
        <?php elseif(!empty($error["pass2"]) && $error['pass2'] != 'blank' && $error['pass2']== 'format'): ?>
                <p class="error">※パスワード確認は半角英数字8～20文字以内で入力してください。</p>
        <?php endif ?>
        <?php if(!empty($_POST["pass2"]) && $error['password'] != 'blank' && $error['pass2'] != 'blank' && $error['password'] == 'mismatch' ): ?>
                <p class="error">※パスワードが一致しません。</p>
        <?php endif ?>
    </div>
    <div class="email form_item">
      <p class="form_item_label">メールアドレス</p><input type="text" name="email" class="form_item_input form_item_input2" value="<?php if( !empty($_POST['email']) ){ echo $_POST['email']; } ?>">    
    </div>
